...
// QR-Code lokal via qrcode-generator
function generateQRCanvas(text, size) {
  const container = document.getElementById('qr-code');
  container.innerHTML = '<p style="color:#8890b0;font-size:.75rem;padding:8px">Lade QR…</p>';
    ...
    try {
      ...
      for (let row = 0; row < moduleCount; row++) {
        for (let col = 0; col < moduleCount; col++) {
          ctx.fillStyle = qr.isDark(row, col) ? '#1a1d27' : '#ffffff';
      ...
      container.innerHTML = `<p style="color:#ff6584;font-size:.7rem;word-break:break-all">${text}</p>`;
    }
  };
  script.onerror = () => {
    container.innerHTML = `<p style="color:#ff6584;font-size:.7rem;word-break:break-all">${text}</p>`;
...
generateQRCanvas(studentURL, 160);
...
onValue(ref(db, 'users'), snap => {
  ...
  snap.forEach(child => { currentUsers[child.key] = child.val(); });
...
document.getElementById('pair-btn').onclick = async () => {
  ...
  if (users.length < 2) { alert('Mindestens 2 Schüler:innen müssen online sein!'); return; }
